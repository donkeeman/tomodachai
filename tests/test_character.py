...
def test_character_creation():
    ...
    assert c.id == "char_1"
    assert c.name == "민수"
    assert c.personality_code == "nori_dynamo"


def test_character_defaults():
    c = Character(id="char_2", name="지은", personality_code="nagomi_dreamer")
...
def test_character_equality():
    a = Character(id="1", name="A", personality_code="nori_dynamo")
    b = Character(id="1", name="A", personality_code="nori_dynamo")
    assert a == b


def test_character_different_ids():
    a = Character(id="1", name="A", personality_code="nori_dynamo")
    b = Character(id="2", name="A", personality_code="nori_dynamo")
    assert a != b
...
def test_speech_habit_backward_compat():
    ...
    c = Character(
        id="1", name="A", personality_code="nori_dynamo",
        speech_habit="~인 거지",
    )
